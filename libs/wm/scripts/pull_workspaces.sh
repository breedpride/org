...
for dir in "$target_dir"/*/; do
  ...
  if [[ -f "$dir/wmill.yaml" ]]; then
    ...
    # Pull the workspace with includes and excludes
    wmill sync pull --includes "$includes" --excludes "$excludes" || {
      echo "Failed to pull workspace: $workspace_name"
      continue
    }

    echo "Successfully pulled workspace: $workspace_name"
  else
